...
gz_model_inputs <-wb_read(gz_scenario,sheet='model_inputs')
head(gz_model_inputs)
error_plot(gz_model_inputs$hhs23,gz_model_inputs$households,xl='hh23',yl='households')
#
# now load up these data in the right format to run the model
#
# make sure the fitting data is numeric
#
for(i in 2:length(names(gz_model_inputs))){
  suppressWarnings(gz_model_inputs[[names(gz_model_inputs)[i]]]<-as.numeric(gz_model_inputs[[names(gz_model_inputs)[i]]]))
}
error_plot(gz_model_inputs$hhs23,gz_model_inputs$households,xl='hh23',yl='households')
#
...
#
names(gz_model_inputs)
hts_ind<-gz_model_inputs[c('geoid',ind_vars$variable)]
...
# save shape file 
#
write_sf(ht_model, "./tiger/sacog/ht_model.shp")
