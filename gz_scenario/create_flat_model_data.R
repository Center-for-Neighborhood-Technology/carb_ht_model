...
#
blkgrp_flat_projections<-wb_read(gz_scenario,sheet='blkgrp_flat_projections')
#
...
blkgrp_flat_projections<-merge(blkgrp_flat_projections,subset(blkgrps,select=c('geoid','lacres')))
#
# get the base fitting inputs
#
model_inputs<-wb_read(carb_ht_model,sheet='fitting_data')
names(model_inputs)[1]<-'geoid'
model_inputs<-subset(model_inputs,model_inputs$geoid %in% blkgrp_flat_projections$geoid)
model_inputs<-subset(model_inputs,
                     select = c('geoid',names(model_inputs)[7:length(names(model_inputs))]))
model_inputs<-merge(blkgrp_flat_projections,model_inputs)
names(model_inputs)
...
if(calc_all_transit_vars){
  ...
  add_xls_tab(gz_scenario,'tas_flat_jobs',tas_jobs)
...
if(do_gravity){
  ...
  # save xlsx sheet
  #
  gz_scenario$remove_worksheet("gravity_flat_10")
  gz_scenario$add_worksheet("gravity_flat_10")
  gz_scenario$add_data("gravity_flat_10",as.data.frame(st_drop_geometry(gravity_10)),colNames = TRUE, rowNames = TRUE,)
  
  wb_save(gz_scenario,file="./excel_files/gz_scenario.xlsx",overwrite = TRUE)
}
gravity_10<-wb_read(gz_scenario,sheet='gravity_flat_10')
#
...
names(model_inputs)
head(model_inputs)
add_xls_tab(gz_scenario,'model_flat_inputs',model_inputs)
wb_save(gz_scenario,file="./excel_files/gz_scenario.xlsx",overwrite = TRUE)
model_inputs_geo<-merge(model_inputs,subset(blkgrps,select=c('geoid','geometry')))
st_write(model_inputs_geo, "./tiger/gz/model_flat_inputs_geo.geojson",append=FALSE, driver = "GeoJSON")
