...
#
blkgp_projections<-wb_read(pda_scenario,sheet='blkgp_projections')
#
# get the clusters for each block group
# 
cal_clusters<-wb_read(pda_scenario,sheet='cal_clusters')
names(cal_clusters)[1]<-'geoid'
blkgp_projections<-merge(blkgp_projections,cal_clusters)
#
# get the base fitting inputs
#
model_inputs<-wb_read(carb_ht_model,sheet='fitting_data')
names(model_inputs)[1]<-'geoid'
model_inputs<-subset(model_inputs,select = c('geoid',names(model_inputs)[7:length(names(model_inputs))]))
...
if(calc_all_transit_vars){
  ...
  source("./utilities/overlaps_and_splits.R")
  tas_jobs <- sum_var_using_fractions(pda_scenario,'tas','tas_geoid','blkgrps','geoid',
                                      model_inputs,'jobs50')
  pda_scenario<-wb_load("./excel_files/pda_scenario.xlsx")
  head(tas_jobs) 
  length(tas_jobs$tas_geoid)
  add_xls_tab(pda_scenario,'tas_jobs',tas_jobs)
  wb_save(pda_scenario,file="./excel_files/pda_scenario.xlsx",overwrite = TRUE)
}
tas_jobs<-wb_read(pda_scenario,sheet='tas_jobs')
...
names(model_inputs)
head(model_inputs)
add_xls_tab(pda_scenario,'model_inputs',model_inputs)
wb_save(pda_scenario,file="./excel_files/pda_scenario.xlsx",overwrite = TRUE)
model_inputs_geo<-merge(model_inputs,subset(blkgrps,select=c('geoid','geometry')))
st_write(model_inputs_geo, "./tiger/mtc/model_inputs_geo.geojson",append=FALSE, driver = "GeoJSON")
