...
#
# get the original dependent variables
#
...
#  this takes a long time, so only do it if you have to.
#
...
if(calc_all_transit_vars){
  ...
  pda_scenario<-add_xls_tab(pda_scenario,'tas_jobs',tas_jobs)
}
...
if(do_gravity){
  ...
  # save xlsx sheet
  #
  pda_scenario<-add_xls_tab(pda_scenario,'gravity_10',as.data.frame(st_drop_geometry(gravity_10)))
}
